...
app.get("/random",(req,res)=>{
  const randnomjoke = Math.floor(Math.random()*jokes.length);
  res.json(jokes[randnomjoke]);
});
...
app.get("/filter",(req,res)=>{
  ...
  const foundactivities = jokes.filter((joke)=> joke.jokeType === type);
...
app.post("/jokes/:id",(req,res)=>{
  ...
  jokes[searchindex] = jokereplacement;
  res.json(jokereplacement);
...
app.delete("/jokes/:id",(req,res)=>{
  ...
  if (searchindex > -1) {
    ...
    res.status(404).json({error : `joe with id ${id} not found. No jokes were deleted`});
